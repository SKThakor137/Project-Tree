/**
 * Self-contained HTML project tree generator with interactive search, folder-first explorer, and customizable exports.
 */
'use strict';
...
/**
 * Generate a self-contained interactive HTML dashboard & exporter.
 * @param {ScanNode} tree
 * @param {Object}   [stats]
 * @returns {string} html
 */
function toHtml(tree, stats = null) {
  ...
  function renderNode(node, depth = 0) {
    ...
    const sortedChildren = sortChildrenFoldersFirst(node.children);
    const childHtml = sortedChildren.map(c => renderNode(c, depth + 1)).join('\n');
  ...
  const archData = (effectiveStats && effectiveStats.architectureGraph)
    ? effectiveStats.architectureGraph
    : { imports: {}, exports: {}, usage: {}, deadCode: { files: [], components: [] }, circular: [] };

  const treeText = buildTreeText(tree, { theme: 'unicode' });
  const treeTextEmoji = buildTreeText(tree, { theme: 'emoji' });
  const treeTextAscii = buildTreeText(tree, { theme: 'ascii' });
  const treeTextBox = buildTreeText(tree, { theme: 'box' });
...
<head>
  ...
  <title>${tree.name} — Project Dashboard & Architecture</title>
  <meta name="description" content="${tree.name} — Interactive Project Intelligence, Mind Map & 2D/3D Code Architecture Visualizer Dashboard.">
  <meta property="og:title" content="${tree.name} — Codebase Intelligence Dashboard">
  <meta property="og:description" content="Explore ${tree.name} codebase structure, dependencies, exports, and architecture. Zero dependencies.">
  ...
  <script type="application/ld+json">
  {
    ...
    "description": "Enterprise Project Intelligence, Mind Maps & 2D/3D Code Architecture Visualizer Suite for Node.js.",
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn {
      ...
      padding: 0.45rem 0.9rem; border-radius: 6px; font-size: 0.85rem; font-weight: 500;
    ...
    .search-input {
      ...
      border-radius: 6px; padding: 0.55rem 0.75rem; color: var(--text); outline: none; font-size: 0.88rem;
    ...
    .tab {
      ...
      flex: 1; text-align: center; font-weight: 500; transition: color 0.15s, border-color 0.15s;
    }
    .tab:hover { color: var(--text); }
    .tab.active { color: var(--accent); border-bottom: 2px solid var(--accent); font-weight: 600; }

    /* Scrollable Areas */
    .tree-container { flex: 1; overflow-y: auto; padding: 1rem 0.75rem; }
    .panel-container { flex: 1; padding: 2rem; overflow-y: auto; }

    /* Tree Styling - VS Code Style (Folders First) */
    ul { list-style: none; padding-left: 1rem; }
    li { margin: 2px 0; }
    .file { cursor: pointer; padding: 4px 8px; border-radius: 4px; display: flex; align-items: center; font-size: 0.9rem; word-break: break-all; }
    ...
    details > summary { cursor: pointer; font-weight: 600; padding: 4px 8px; border-radius: 4px; outline: none; font-size: 0.9rem; color: var(--text); }
    details > summary:hover { background: var(--bg-hover); }

    .hidden { color: var(--danger); font-style: italic; }
    .summary { color: var(--text-muted); font-size: 0.82em; margin-left: 8px; font-weight: normal; }
    ...
    .export-header-banner {
      background: var(--card-bg); border: 1px solid var(--border); padding: 1.25rem 1.5rem;
      border-radius: 10px; margin-bottom: 1.5rem; display: flex; align-items: center;
      justify-content: space-between; flex-wrap: wrap; gap: 1rem; box-shadow: var(--shadow);
    }
    .export-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(270px, 1fr)); gap: 1.25rem; margin-top: 1rem; }
    .export-card {
      background: var(--card-bg); border: 1px solid var(--border); padding: 1.25rem;
      border-radius: 10px; display: flex; flex-direction: column; justify-content: space-between; gap: 1.2rem;
    ...
    .export-card .btn { width: 100%; justify-content: center; text-align: center; }
    ...
    .select-theme-dropdown {
      ...
      padding: 0.35rem 0.6rem; border-radius: 6px; font-size: 0.82rem; outline: none; margin-bottom: 0.5rem; width: 100%;
    }

    .shortcuts-bar { font-size: 0.78rem; color: var(--text-muted); display: flex; gap: 1.25rem; padding: 0.5rem 1.25rem; background: var(--bg-panel); border-top: 1px solid var(--border); }
    kbd { background: var(--bg-hover); border: 1px solid var(--border); padding: 1px 6px; border-radius: 4px; font-family: monospace; font-size: 0.75rem; }
...
<body>
  ...
  <div class="app-body">
    ...
    <div class="sidebar">
      <div class="sidebar-header">
        <div class="search-wrapper">
          <input type="text" id="treeSearch" class="search-input" placeholder="Search files in project... (/)" />
        </div>
      </div>

      <div class="tabs">
        <div class="tab active" data-target="tree">Explorer</div>
        <div class="tab" data-target="overview">Dashboard & Exports</div>
      </div>

      <!-- Explorer Tab: File Tree (Folders First) -->
      <div class="tree-container" id="treeView">
        <ul>${treeHtml}</ul>
      </div>

      <!-- Overview Stats Sidebar Tab -->
      <div class="tree-container" id="overviewView" style="display: none;">
        <p style="color: var(--text-muted); font-size: 0.88rem; margin-bottom: 1rem;">
          Select a file from <b>Explorer</b> to inspect code properties and component dependencies.
        </p>

        ${effectiveStats ? `
          <div class="stat-card" style="margin-bottom: 0.8rem;">
            <div class="stat-val">${effectiveStats.files}</div>
            <div class="stat-label">Total Files (${effectiveStats.dirs} Folders)</div>
          </div>
          <div class="stat-card" style="margin-bottom: 0.8rem;">
            <div class="stat-val">${effectiveStats.totalSizeText}</div>
            <div class="stat-label">Repository Code Size</div>
          </div>
          <div class="stat-card">
    ...
    <div class="main">
      <div class="panel-container" id="mainPanel">
        ...
        <!-- Export & Report Download Section -->
        <div id="exportSection">

          <div class="export-header-banner">
            <div>
              <h3 style="color: var(--accent); margin-bottom: 0.25rem;">📦 Export Selected Reports to ZIP</h3>
              <p style="color: var(--text-muted); font-size: 0.88rem;">Select report checkboxes below and download your customized ZIP archive instantly.</p>
            </div>
            <div style="display: flex; gap: 0.6rem; align-items: center;">
              <button class="btn btn-outline" id="toggleSelectBtn" onclick="handleToggleSelectAll()">Deselect All</button>
              <button class="btn btn-primary" style="padding: 0.55rem 1.1rem; font-size: 0.9rem;" onclick="downloadSelectedZipBundle()">
          ...
          <div class="export-grid">
            ...
            <div class="export-card">
              <div>
                ...
                <p>Standard ASCII/Unicode tree structure with code blocks and file stats.</p>
              </div>
              <button class="btn btn-outline" onclick="downloadReport('PROJECT_STRUCTURE.md')">Download MD</button>
            </div>
            ...
            <div class="export-card">
              <div>
                ...
                <p>Full JSON tree representation with deep directory structure and file metadata.</p>
              </div>
              <button class="btn btn-outline" onclick="downloadReport('PROJECT_STRUCTURE.json')">Download JSON</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('PROJECT_STRUCTURE.mmd')">Download Mermaid</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('AI_CONTEXT.md')">Download AI Context</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('COMPONENT_USAGE.json')">Download Component Report</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('IMPORT_GRAPH.json')">Download Dependency Graph</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('DEAD_CODE.json')">Download Dead Code Report</button>
            </div>
            ...
            <div class="export-card">
              ...
              <button class="btn btn-outline" onclick="downloadReport('PROJECT_HEALTH.json')">Download Health Metrics</button>
            </div>
  ...
  <div class="shortcuts-bar">
    <span>Shortcuts:</span>
    <span><kbd>/</kbd> Search files</span>
  ...
  <script>
    ...
    function downloadFile(filename, content, type = 'text/plain') {
      if (!content) {
        alert('Report data is empty for ' + filename);
        return;
      }
    ...
    function downloadReport(key) {
      if (CLIENT_REPORTS[key]) {
        ...
        alert('File ' + key + ' not found in report package.');
      }
    }

    function downloadSelectedSvg() {
      const selectedKey = document.getElementById('svgThemeSelect').value;
      downloadReport(selectedKey);
    }

    function handleToggleSelectAll() {
      allSelectedState = !allSelectedState;
      document.querySelectorAll('.report-select-cb').forEach(cb => cb.checked = allSelectedState);
      const btn = document.getElementById('toggleSelectBtn');
      btn.textContent = allSelectedState ? 'Deselect All' : 'Select All';
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Search Filter - Auto Switch to Explorer View & Realtime Expand
      searchInput.addEventListener('input', (e) => {
        const query = e.target.value.toLowerCase().trim();
        
        // Auto-switch to Explorer tree tab on active typing
        if (query.length > 0) {
          tabs.forEach(t => t.classList.remove('active'));
          document.querySelector('.tab[data-target="tree"]').classList.add('active');
          treeView.style.display = 'block';
          overviewView.style.display = 'none';
        }
      ...
      // File Details Panel Selection
      files.forEach(f => {
        f.addEventListener('click', (e) => {
          files.forEach(el => el.classList.remove('active'));
          f.classList.add('active');

          const relPath = f.dataset.path || f.textContent.trim();
